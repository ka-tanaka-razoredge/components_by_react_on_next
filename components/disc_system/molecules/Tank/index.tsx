...
import MutableSail from '@/components/disc_system/MutableSail';
import Cable from './Cable';
import Carousel from '@/components/Carousel';
import PastOrFuture from './PastOrFuture';
import Wings from '@/components/disc_system/molecules/Wings';
...
//console.log(discs);
  ...
  useEffect(() => {
    ...
    tank.current.addEventListener('pushDisc', (e) => {
      pushDisc(e.detail);
    ...
    tank.current.addEventListener('forwardCurrentIndex', (e) => {
      forwardCurrentIndex(e.detail);
    });

    tank.current.addEventListener('moveY', (e) => {
      return moveY(e.detail);
    });
    
    tank.current.addEventListener('disc resized', (e) => {
//console.log(e);
      tank.current.shiftXThemEx({left: e.detail.left, value: e.detail.value, width: e.detail.width});
    });

    //--------------------------------------------------------------------------------
    // begin ooRef provider only
    //--------------------------------------------------------------------------------
    tank.current.getDiscs = () => {
      return discsRef.current;
    };
    
    tank.current.setDisc = (value) => {
    };
...
//      console.log(JSON.parse(lop.value, null, 2));
    ...
    tank.current.quadraticCurveTo = (lop: { beginVector: Vector, imaginaryVectorFirst: Vector, endVector: Vector }) => {
...
//        console.log('---- quadraticCurveTo end ----');
    ...
    tank.current.drawText = (lop) => {
      let context = document.getElementById('fore-canvas').getContext('2d');
      context.fillText(lop.text, lop.x, lop.y);
    };

    tank.current.drawFilledTriangle= (lop: { top: Vector, left: Vector, right: Vector }) => {
      let context = document.getElementById('fore-canvas').getContext('2d');
      context.beginPath();
    ...
    <div style={{ position: 'relative', top: `${top}px` }}>
      {
        props.foreCanvas && (<div style={{ position: 'absolute', backgroundColor: 'rgba(200, 200, 200, 0.1)', width: '50vw', height: '100vh', zIndex: 1000, left: 0, top: 0 }}>
          <canvas id='_fore-canvas' width={width} height={height}></canvas>
        </div>)
      }
      <div
        ref={tank}
        style={{
          ...
          position: 'relative',
          left: left + 'px',
          transform: `rotateY(${40}deg) rotateX(60deg) translateX(-96px) translateY(50px)`,
        }}
      >
